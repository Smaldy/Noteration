...
# Schedule sources that the user owns — never regenerated by an SM-2 rebuild.
# ``manual`` = custom events, topic/subject sessions, and drag-drop reschedules;
# ``ai`` = entries placed by the AI study planner.
_PRESERVED_SOURCES = (ScheduleSource.manual, ScheduleSource.ai)
...
def rebuild_schedule(
    ...
    exam day) are flagged ``is_revision_buffer``. User-owned entries (``manual``
    drag-drops/custom events and ``ai`` plan entries) are preserved. A regenerated
    (topic, date) inherits any completion checkmark that was on the old auto entry
    for the same slot, so checking off a review survives the next rebuild. Does
    not commit.
    """
    ...
    # Snapshot completion on the auto entries we're about to replace so a checked
    # review isn't silently un-checked when the calendar is rebuilt.
    stale = session.scalars(
        select(ScheduleEntry).where(
            ScheduleEntry.topic_id.in_(topic_ids),
            ScheduleEntry.source.not_in(_PRESERVED_SOURCES),
            ScheduleEntry.completed.is_(True),
        )
    ).all()
    done_slots = {(e.topic_id, e.date): e.completed_at for e in stale}

    # Replace only machine-generated entries; user-owned ones survive.
    session.execute(
    ...
    for topic_id in topic_ids:
        ...
        for due in sorted(set(due_dates)):
            ...
            was_done = (topic_id, due) in done_slots
            entry = ScheduleEntry(
                topic_id=topic_id,
                date=due,
                source=source,
                ...
                completed=was_done,
                completed_at=done_slots.get((topic_id, due)),
            )
            session.add(entry)
