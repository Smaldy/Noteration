...
## Exercise Duplicator (feature — Option C three-stage hybrid pipeline)

New `/duplicator` page in Exam Prep: upload an exercise PDF → Stage 1 extracts
each exercise (sync vision) → Stage 2 searches real university-level variants per
exercise (background queue) → Stage 3 renders visualizations (frontend, on-demand).

- **Wave ED-1 — Data models + migration (DONE).** `backend/models/duplicator.py`:
  `ExerciseSession` (document_hash = ingestion cache key, year_level 1–5,
  subject_hint, status extracting|ready|error), `ExtractedExercise`
  (session FK CASCADE, order_index, raw_text, topic dot-notation, subtopic,
  `difficulty_signals` JSON list, `viz` JSON nullable, status
  pending|searching|done|error), `DuplicateResult` (exercise FK CASCADE,
  source_url, problem_text, `viz` JSON, difficulty_score, `queue_job_id` FK →
  queue_jobs SET NULL), `CalibrationSample` (topic/subtopic/year_level/source_text,
  source own|imported). Three `StrEnum`s added to `enums.py`; all four models
  re-exported from `models/__init__.py`. One autogenerated migration
  (`56220802d7c5_duplicator_tables`), reviewed (added the `import backend.db.types`
  the autogen omits), applied; `alembic check` = no drift; downgrade/upgrade
  round-trips. 9 tests (round-trip each table, viz-block JSON, session→exercises→
...
- **3 pre-existing test failures are environmental, not regressions.**
  ...
  `date.today()` against the service's `utcnow().date()`; right now (early local
  morning) UTC is one calendar day behind local, so they fail. They pass when
  local and UTC dates realign. Untouched by this feature; "green" for ED waves =
  the 546 baseline stays green + new tests pass.
- **`CalibrationSource.imported = "imported"` (not the spec's bare `"import"`).**
  `import` is a Python keyword (illegal enum member name), and the repo-wide
  convention is `StrEnum` member name == value with `SAEnum` storing the *name*
  (no `values_callable` anywhere). Kept that invariant; the wire string isn't
  load-bearing as long as calibration export/import round-trips through the enum.
